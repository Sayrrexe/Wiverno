...
class TestBaseViewIntegration:
    """Integration tests for BaseView with Wiverno application."""

    def test_base_view_in_wiverno_app(self, call_wsgi_app, environ_factory):
        """Test: BaseView works in Wiverno application."""
        from wiverno.main import Wiverno

        class MyView(BaseView):
            def get(self, request):
                return "200 OK", "<h1>Home Page</h1>"

            def post(self, request):
                return "201 Created", "<h1>Created</h1>"

        app = Wiverno()
        view = MyView()

        # Register view as route handler
        app._routes["/home"] = {"handler": view, "methods": None}

        # Test GET
        environ = environ_factory(path="/home")
        status, headers, body = call_wsgi_app(app, environ)
        assert status == "200 OK"
        assert "Home Page" in body

        # Test POST
        environ = environ_factory(method="POST", path="/home")
        status, headers, body = call_wsgi_app(app, environ)
        assert "201" in status

    def test_multiple_views_in_app(self, call_wsgi_app, environ_factory):
        """Test: Multiple BaseView instances in application."""
        from wiverno.main import Wiverno
        ...
        class ContactView(BaseView):
            def get(self, request):
                return "200 OK", "<h1>Contact</h1>"

            def post(self, request):
                return "200 OK", "<h1>Message Sent</h1>"

        app = Wiverno()
        app._routes["/"] = {"handler": HomeView(), "methods": None}
        app._routes["/about"] = {"handler": AboutView(), "methods": None}
        app._routes["/contact"] = {"handler": ContactView(), "methods": None}
        ...
        environ = environ_factory(path="/contact")
        status, headers, body = call_wsgi_app(app, environ)
